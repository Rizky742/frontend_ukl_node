    ...
    return (
        <div className='relative w-full shadow bg-white'>
            <div className='container sm:flex m-auto sm:justify-between items-center text-gray-700'>
                <NavLink to="/"><h1 className='group pl-8 py-4 text-xl font-semibold cursor-pointer bg-white relative z-10'>CobaWeb</h1></NavLink>
                <ul className={`text-center transition-all duration-200 ease-out relative ${open ? 'pb-4 sm:pb top-0 h-min' : '-top-[300px] h-0'} sm:flex sm:static cursor-pointer items-center text-base`}>
                    {/* {list.map((res) => (
                        ...
                        <NavLink to={'/outlet'}><li className={`px-2 py-2 hover:text-blue-400  ${sessionStorage.getItem('role') === 'admin' || sessionStorage.getItem('role') === 'kasir'  ? "" : "hidden"}`}>Outlet</li></NavLink>
                        ...
                        <NavLink to={'/transaksi'}><li className={`px-2 py-2 hover:text-blue-400`}>Transaksi</li></NavLink>
                        <NavLink to={'/laporan'}><li className={`px-2 py-2 hover:text-blue-400 ${sessionStorage.getItem('role') === 'admin' ? "" : "hidden"}`}>Laporan</li></NavLink>
                ...
                <Menu onClick={() => setOpen(!open)} className='text-black absolute top-4 right-6 sm:hidden cursor-pointer z-20' height={"2rem"} />
            </div>
        </div>
    )
}
